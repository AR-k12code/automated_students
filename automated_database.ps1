...
if ($SkipDownloadingReports -eq $False) {
  ...
  . $currentPath\..\CognosDownload.ps1 -username $eSchoolUsername -espdsn $eSchooldsn -report FAKE -EstablishSessionOnly
  ...
  #Establishing session by dot sourcing the CognosDownloader causes $currentPath to be overwritten. Set again to correct.
  $currentPath=(Split-Path ((Get-Variable MyInvocation -Scope 0).Value).MyCommand.Path)

  $results = $reports.keys | ForEach-Object -Parallel {

    $incomingsession = $using:session
    $report = ($using:reports).$PSItem

    write-host "INFO: Invoking Cognos Downloader script to pull $PSItem"
    
    if ($using:UseSharedCognosFolder) {
      $cognoscmd = "$using:currentPath\..\CognosDownload.ps1 -username ""$using:eSchoolUsername"" -espdsn ""$using:eSchooldsn"" -report ""$PSItem"" -cognosfolder ""_Share Temporarily Between Districts/Gentry/automation-210408"" -TeamContent -SessionEstablished -savepath ""$using:currentPath\files\"" -reportparams ""$($report.parameters)"" -TrimCSVWhiteSpace"
    } else {
      $cognoscmd = "$using:currentPath\..\CognosDownload.ps1 -username ""$using:eSchoolUsername"" -espdsn ""$using:eSchooldsn"" -report ""$PSItem"" -cognosfolder ""automation"" -SessionEstablished -savepath ""$using:currentPath\files\"" -reportparams ""$($report.parameters)"" -TrimCSVWhiteSpace"
    }

    Invoke-Expression $cognoscmd

    #Activites is sometimes blank for smaller districts. If the first try fails with a CSV verification failure it might just be blank. Run again if we got a 8,9, or 10 return code and hopefully it will succeed and return 0.
    if (($PSItem -eq "activities") -AND (@(8,9,10) -contains $LASTEXITCODE)) {
      $cognoscmd += " -DisableCSVVerification"
      Invoke-Expression $cognoscmd
    }
  ...
  $failedJobs | ForEach-Object {
      $errorMessageString += $PSItem | Receive-Job
  }
  ...
  if (($failedJobs | Measure-Object).count -ge 1) {
      ...
      if ($sendMailNotifications) {
        Send-EmailNotification -subject "Automated_Students: Cognos Report Failure" -body "There were $(($failedJobs | Measure-Object).count) errors detected while downloading Cognos Reports.`r`n`r`n$($errorMessageString)`r`n`r`nPlease inspect the log file $($logfile) for more details.`r`n"
      }
  }
...
"@
  ...
  if ($homeroomsScheduled) {
    ...
    #Invoke-SqlUpdate -Query 'DELETE FROM students_extras WHERE Timestamp != (SELECT DISTINCT Timestamp FROM students_extras ORDER BY Timestamp DESC LIMIT 1);'
  
  
    $homeroomQuery = Invoke-SqlQuery -Query "select DISTINCT Students.School_id,Student_homeroom,Student_hrmtid,Teacher_email,teachers.First_name,teachers.Last_name `
    ...
    $homeroomQuery | ForEach-Object {
      ...
      #Adding "AND Student_id in (SELECT Student_id FROM students)"" is to ensure its only active students. students_extras are never cleaned up currently.
      $studentids = Invoke-SqlQuery -Query "SELECT Student_id FROM students_extras WHERE Student_homeroom = $homeroom AND Student_id IN (SELECT Student_id FROM students)" | Select-Object -ExpandProperty Student_id
      
      $studentids | foreach-object {
        $query = "(School_id, Section_id, Student_id, Marking_period, Timestamp) VALUES ($schoolid,$classid,$PSItem,1,$timestamp),($schoolid,$classid,$PSItem,2,$timestamp),($schoolid,$classid,$PSItem,3,$timestamp),($schoolid,$classid,$PSItem,4,$timestamp)"
        if (@('mysql','mariadb') -contains $database.dbtype) {
          $query = 'INSERT INTO `enrollments`',$query,"ON DUPLICATE KEY UPDATE `Timestamp`=$timestamp"
          Invoke-SqlUpdate -Query $query | Out-Null
        } else {
          $query = 'INSERT OR REPLACE INTO `enrollments`',$query
          Invoke-SqlUpdate -Query $query | Out-Null
        }
      }
    ...
    $homerooms | ForEach-Object {
      ...
      try {
        ...
        write-host "Error: Could not import homerooms correctly. $PSItem" -ForegroundColor RED
        exit(1)
      }
    }
  ...
  if ($activitiesScheduled) {
    ...
    try { Invoke-SqlUpdate -Query 'DELETE FROM activities WHERE Timestamp != (SELECT * FROM (SELECT DISTINCT Timestamp FROM activities ORDER BY Timestamp DESC LIMIT 1) AS X);' | Out-Null } catch {}
  
    $activitiesQuery = Invoke-SqlQuery -Query "SELECT DISTINCT School_id,Teacher_id,Activity_code,Activity_name from activities"
    if ($null -eq $activitiesQuery) {
      ...
      $activities | ForEach-Object {
        ...
        try {
          ...
          write-host "Error: Could not build activities correctly. $PSItem" -ForegroundColor RED
          exit(1)
        }
      }
